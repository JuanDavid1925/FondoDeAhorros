...
const TabPagoCuotaManejo = ({ router }) => {
  const { getCuotaManejoPendiente, pagoManejo } = useSaves()
  ...
  const handleSubmitManejo = useCallback(data => {
    if (data.cantidadPagarManejo == "") {
      pagoManejo(cuotaManejo, setEstado)
    }
    else {
      pagoManejo(data.cantidadPagarManejo, setEstado)
    }
  }, [pagoManejo, cuotaManejo])
  ...
  useEffect(() => {
    if (!cuotaManejo) {
      getCuotaManejoPendiente(setCuotaManejo)
    }
    else {
      $("#cantidadPagarManejo").val(cuotaManejo)
    }
  }, [cuotaManejo, getCuotaManejoPendiente])
              ...
              <div className="flex flex-col sm:flex-row">
                <div className="relative flex-shrink-0 sm:w-7/12">
                  <input
                    onChange={handleChange}
                    type="text"
                    id="cantidadPagarManejo"
                    name="cantidadPagarManejo"
                    className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500"
                    placeholder=" "
                  />
...
export default withRouter(TabPagoCuotaManejo)
